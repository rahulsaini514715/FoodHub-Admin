...
const ProductsScreen = () => {
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err) => {
      Alert.alert("Error", "Product delete nahi hua");
      console.log(err);
    },
  });
  ...
  const renderProduct = ({ item }: { item: Product }) => (
    <View className="bg-white rounded-xl p-4 mx-3 my-2 flex-row item-start shadow">
      <Image
      ...
      <View className="flex-row items-center">
        ...
        <TouchableOpacity onPress={() => handleDelete(item?.id, item?.name)} className="p-2 rounded-md bg-red-50">
  ...
  return (
    <View className="flex-1 bg-gray-50">
      <View className="flex-row items-center justify-between px-4 py-4">
        <Text className="txt-xl font-bold text-gray-900">
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text className="text-center text-gray-500 mt-8">No Products Found</Text>}
        />
      )
      }
    </View>
